                ...
                try
                {
                    ...
                    if (dev.TryOpen(out hidStream))
                    {
                        ...
                        using (hidStream)
                        {
                            ...
                            var cmd = System.Text.Encoding.ASCII.GetBytes("QPIRI");
                            ...
                            {
                                Console.WriteLine(toWrite[i].ToString("X2"));
                            }
                            ...
                            var buf = new byte[10];
                            int totalRead = hidStream.Read(buf, 0, buf.Length);
                            while (totalRead <= 109)
                            {
                                Console.Write(System.Text.Encoding.ASCII.GetString(buf));
                                totalRead += hidStream.Read(buf, 0, buf.Length);
                            }
                            Console.WriteLine();
                            cmd = System.Text.Encoding.ASCII.GetBytes("QPIGS");
                            crc = BitConverter.GetBytes(Crc16.ComputeChecksum(cmd));
                            toWrite = new byte[cmd.Length + crc.Length + 1];
                            System.Buffer.BlockCopy(cmd, 0, toWrite, 0, cmd.Length);
                            System.Buffer.BlockCopy(crc, 0, toWrite, cmd.Length, crc.Length);
                            toWrite[toWrite.Length - 1] = 0x0d;
                            for (int i = 0; i < toWrite.Length; i++)
